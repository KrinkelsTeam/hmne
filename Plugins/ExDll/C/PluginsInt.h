...
#define TBITEM_STATE_ENABLED  0x00000001
#define TBITEM_STATE_CHECKED  0x00000002
#define TBITEM_STATE_VISIBLE  0x00000004

// PluginData.Command return this on command failure.
#define COMMAND_FAIL -1

  // Version number
#define HMNE_VERSION 0x00000001


   // This struct represents a toolbar item
typedef struct
{
     char *Name;
...
typedef TBItemData *PTBItemData;
...
typedef struct
{
     ...
     int (*NotifyProc)(Event Event, int Param1, int Param2, int Param3);
     // Called when some events occurs. Can be null. See Event enum type for a
     // list of possible notifications.

     int (*Command)(EditorCommand Command, int Param1, int Param2, int Param3);
     // Command procedure (filled by the program). Call this to perform operations
     // in the editor, see the EditorCommand enum type for a list of available commands.
     // NOTE: if any command fails this functions returns COMMAND_FAIL
} PluginData;

typedef PluginData *PPluginData;
